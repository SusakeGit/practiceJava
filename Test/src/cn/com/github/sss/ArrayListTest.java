...
public class ArrayListTest {
	...
	/**
	 * 测试ArrayList的添加元素方法�?�以及与size有关的方�?
	 */
	public static void testArrayListSize(){
		...
		list3.add("j");
		list3.add("k");
		list.addAll(list2);//从list末尾�?始追�?
		System.out.println(list.size());// result: 6
		list.addAll(6, list3);//从list索引6�?始添�?
		...
		list.trimToSize();//将list的大小设置成与其包含的元素相同�?�null也算是list中的元素、并且可以重复出�?
		System.out.println(list.size());
		list.ensureCapacity(1);//确保list的大小不小于传入的参数�?��??
		System.out.println(list.size());
		System.out.println(list.size());
	...
	public static void testArrayListContainsRemove(){
		...
		//查看list1中是否包含学号为1的学生（ 这里要注意�?�ArrayList中存放的都是对象的引用�?��?�非堆内存中的对象）
		System.out.println(list1.contains(s1));
		
		//查看list1中是否包含list2
		System.out.println(list1.containsAll(list2));
		
		//从新构�?�一个指向学号为1的student、查看list2是否包含、不包含就添加进去�?�在判断list1是否包含list2
	...
	public static void testObtainArrayListElements(){
		...
		/*
		使用时会抛异常�?�是由于Arrays.asList(strArray)返回的是�?个Object[]、不能强转成ArrayList<String>类型
		ArrayList<String> list2 = (ArrayList<String>)Arrays.asList(strArray);
		System.out.println(list2);
		*/
		
		//�?般情况下使用下面这种转换方式、他会自动的将数组转换之后的类型设置为runtime时的类型
	...
	public static void testListIterator(){
		...
		while(li1.hasNext()){
			//System.out.println(li1.nextIndex());		会�?�成死循环�?�具体可以看源码
			//System.out.println(li1.previousIndex());		同样会�?�成死循环�?�具体可以看源码
			String s = li1.next();
		...
		//对于在遍历过程中想获取index、要注意死循环�?�和字节想要获取的方式�?�具体可以自己动手试�?
	...
	public static void testArrayList2Array(){
		//关于Array转换成ArrayList上面已经有过介绍、现在再补充�?点特殊情�?
		...
		*/
		...
		//第三种�?�应该使用这种形式的定义、传入的参数的本质是供toArray内部调用其类型�?�对其size�?单处理一下�?�如果size大于list的size、则后面的补null、如果小于�?�则使用新的数组替换传入的�?�并作为结果返回
	...
	public static void testFailFast(){
		...
		while(it.hasNext()){
			...
			//这里本来是多线程动了ArrayList中的元素造成的�?�现在仅仅是模拟�?种情况�?�就是在迭代的过程中、另�?个线程向ArrayList中添加一个元素�?�成的fail-fast
